...
player_images = [
    tk.PhotoImage(file="b disc.png"),  
    tk.PhotoImage(file="w disc.png")   
]
...
def play_song1():
    try:
        pygame.mixer.music.load("Sakura sound.mp3")  
...
def play_song2():
    try:
        pygame.mixer.music.load("epic sound.mp3")  
        pygame.mixer.music.play()
    except pygame.error as e:
        messagebox.showerror("Error", f"Unable to play the song: {e}")
...
def disclaimer():
    return messagebox.askokcancel("Disclaimer", 
                                  "If the player intentionally places a disc between their opponent's discs "
                                  "without outflanking any, the move will be wasted. Do you understand and agree to proceed?")
...
def declare_winner():
    ...
    winner_window.state('zoomed')   

    # Add background image
    winner_bg_image = tk.PhotoImage(file="winner img.png")   
    winner_bg_label = tk.Label(winner_window, image=winner_bg_image)
    winner_bg_label.place(x=0, y=0, relwidth=1, relheight=1)
    winner_bg_label.image = winner_bg_image  
...
def on_cell_click(event):
    ...
    if all(cell is not None for row in board_state for cell in row) or is_game_over():
        messagebox.askokcancel("Exit", "GAME OVER")
        declare_winner()
    else:
...
# Background image
bg_image = tk.PhotoImage(file="green-board.png")   
...
def update_current_player_label():
    current_player_label.config(text=f"Current Player: {'Black' if current_player == BLACK else 'White'}")
...
current_player_label = tk.Label(third_window, text="Current Player: Black", font=("Arial", 30), bg='light yellow')
current_player_label.place(relx=0.05, rely=0.3)

def update_current_player_label():
    global current_player
    player_text = "Black" if current_player == BLACK else "White"
    current_player_label.config(text=f"Current Player: {player_text}")
...
tutorial_text = (
    ...
    "7. When it is no longer possible for either player to move, the game is over. \n    Discs are counted and the player with the majority of their color showing is the winner.\n\n"
